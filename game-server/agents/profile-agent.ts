...
export function computeProfileUpdate(
  ...
  const plateauCount = level > prev.level + tuning.PLATEAU_EPSILON ? 0 : prev.plateauCount + 1;
  ...
  return {
    _ema, level, confidence, sessionsCount, trend, lastDomainScores,
    volatility, plateauCount, deteriorationFlag, bestLevel, bestAt,
  };
}
...
export async function updateCognitiveProfile(
  ...
  for (const [domainId, score] of Object.entries(domainScores) as [ProblemId, number][]) {
    ...
    try {
      ...
      const next = computeProfileUpdate(prev, score, weight, PROFILE_TUNING, now);
      ...
      appendTimelinePoint(userId, domainId, {
        t: now, level: next.level, score, confidence: next.confidence,
      }).catch(() => {});
