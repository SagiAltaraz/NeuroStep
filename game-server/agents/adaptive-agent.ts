...
const D_DEFAULT = 0.4; // cold-start level (no saved history)
const DSMOOTH_ALPHA = 0.3; // smoothing for the persisted D
...
async function loadProfile(
   ...
   if (userId === 'anonymous')
      return { mean: null, stdDev: null, difficulty: null };
   try {
      ...
      const stdDev =
         typeof d.stdDevReactionMs === 'number' ? d.stdDevReactionMs : null;
      const difficulty =
         typeof d.difficultyLevel === 'number' ? d.difficultyLevel : null;
 ...
 */
export function resumeDifficulty(
   state: AdaptiveState
): Promise<DifficultyParams | null> {
 ...
 */
export async function seedLevelFromProfile(
   userId: string,
   gameId: GameId
): Promise<number | null> {
 ...
 */
export function applyWarmupSeed(
   state: AdaptiveState,
   seedLevel: number
): DifficultyParams {
...
export async function processEvent(
   ...
   if (tuning.pMode === 'outcome') {
      ...
      if (event.type === 'GAME_WON')
         state.outcomeWindow.push(event.winner === 'ai' ? 0 : 1);
      else if (event.type === 'GAME_DRAW') state.outcomeWindow.push(0.5);
      else return { adjusted: false };
      if (state.outcomeWindow.length > (tuning.outcomeWindow ?? 8))
         state.outcomeWindow.shift();
      // Outcome games map to the feature stream coarsely: a loss is the "error".
      recordFeature(
         event.type === 'GAME_WON' && event.winner === 'ai' ? 'miss' : 'hit',
         null
      );
      ...
      recordFeature(
         isHit
            ? 'hit'
            : classifyEvent(state.gameId, event.type) === 'timeout'
              ? 'timeout'
              : 'miss',
         isHit &&
            event.reactionMs &&
            event.reactionMs > 0 &&
            event.reactionMs < 10_000
            ? event.reactionMs
            : null
      );
      state.accuracyWindow.push(isHit);
      if (state.accuracyWindow.length > ACCURACY_WINDOW)
         state.accuracyWindow.shift();

      // Reaction-time EMA + trend window (hits only — misses have no clean RT).
      const rt = event.reactionMs ?? null;
      if (rt && rt > 0 && rt < 10_000 && isHit) {
         state.emaReactionMs =
            state.emaReactionMs === null
               ? rt
               : EMA_ALPHA * rt + (1 - EMA_ALPHA) * state.emaReactionMs;
         state.reactionWindow.push(rt);
         if (state.reactionWindow.length > TREND_WINDOW)
            state.reactionWindow.shift();
      }
   }

   // ── Gate: enough data + event-count cadence + anti-burst cooldown ──
   if (state.totalScoredEvents < MIN_EVENTS) return { adjusted: false };
   if (state.totalScoredEvents - state.lastEvalCount < tuning.evalEvery)
      return { adjusted: false };
   if (Date.now() - state.lastAdjustAt < MIN_COOLDOWN_MS)
      return { adjusted: false };
   ...
   state.dSmoothed =
      DSMOOTH_ALPHA * state.D + (1 - DSMOOTH_ALPHA) * state.dSmoothed;
   ...
   state.D = newD;
   state.dSmoothed =
      DSMOOTH_ALPHA * newD + (1 - DSMOOTH_ALPHA) * state.dSmoothed;
...
function speedScore(state: AdaptiveState): number | null {
   return speedVsBaseline(
      state.emaReactionMs,
      state.baselineMean,
      state.baselineStdDev
   );
}
