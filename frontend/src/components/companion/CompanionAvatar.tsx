...
const readMs = (t: string) =>
   Math.min(11000, Math.max(4200, 2400 + t.length * 55));
...
const askedMoodToday = () => {
   try {
      return localStorage.getItem(MOOD_ASK_KEY) === todayStr();
   } catch {
      return false;
   }
};
function inferUserGender(profile: unknown): UserGender {
   ...
   const genderEntry = record?.personalizationQuestionnaire?.find(
      (entry) => entry.questionId === 2
   );
   const selectedIds = Array.isArray(genderEntry?.selectedOptionIds)
      ? genderEntry.selectedOptionIds.filter(
   ...
   const answer = String(
      record?.personalizationAnswers?.[2] ??
         record?.personalizationAnswers?.['2'] ??
         ''
   ).toLowerCase();
...
function adaptHebrewForGender(
   ...
   return text
      .replace(/מרגיש/g, 'מרגישה')
      .replace(/חושב/g, 'חושבת')
      .replace(/יודע/g, 'יודעת')
      .replace(/זוכר/g, 'זוכרת')
      .replace(/מבין/g, 'מבינה')
      .replace(/מכיר/g, 'מכירה')
      .replace(/יכול/g, 'יכולה')
      .replace(/צריך/g, 'צריכה')
      .replace(/בטוח/g, 'בטוחה')
      .replace(/רגוע/g, 'רגועה')
      .replace(/עייף/g, 'עייפה')
      .replace(/ערני/g, 'ערנית')
      .replace(/חזק/g, 'חזקה')
      .replace(/מהיר/g, 'מהירה')
      .replace(/מדויק/g, 'מדויקת')
      .replace(/מרוכז/g, 'מרוכזת')
      .replace(/עקבי/g, 'עקבית')
      .replace(/סבלני/g, 'סבלנית')
      .replace(/מצליח/g, 'מצליחה')
      .replace(/מתקדם/g, 'מתקדמת')
      .replace(/לומד/g, 'לומדת')
      .replace(/ממשיך/g, 'ממשיכה')
      .replace(/משתפר/g, 'משתפרת')
      .replace(/מתמיד/g, 'מתמידה')
      .replace(/משקיע/g, 'משקיעה')
...
function buildMessages(
   ...
   const gameName2 =
      GAME_INSTRUCTIONS[gameKey]?.title[lang] ??
      (lang === 'he' ? data?.suggestedGameHe : undefined) ??
      GAME_INSTRUCTIONS.memory.title[lang];
   return [
      {
         text: adaptHebrewForGender(
            (lang === 'he' ? data?.greetingHe : undefined) ??
               gt('companion.home.greetingFallback'),
            gender,
            lang
         ),
         dir,
      ...
      {
         text: adaptHebrewForGender(
            (lang === 'he' ? data?.reasonHe : undefined) ??
               gt('companion.home.reasonFallback'),
            gender,
            lang
         ),
         cta: {
            label: fmt('companion.home.ctaPlay', { game: gameName2 }),
            gameId,
         },
         dir,
      },
...
function buildFallbackSuggestions(
   ...
   const recommendation: Msg = {
      ...
      cta: {
         label: `${gt('companion.play')} ${gameName}`,
...
function formatSuggestionText(
   ...
   const text = template.replace(
      /\{(\w+)\}/g,
      (placeholder, key: string) => params[key] ?? placeholder
   );
...
function backendSuggestionMessage(
   ...
   const text = adaptHebrewForGender(
      formatSuggestionText(t(messageKey), params) ?? '',
      gender,
      lang
   );
   if (!text) return null;
   ...
   if (suggestion.action) {
      ...
      const label = adaptHebrewForGender(
         formatSuggestionText(t(labelKey), params) ?? '',
         gender,
         lang
      );
      ...
      if (suggestion.action.type === 'open-game') {
         ...
         if (
            !gameId ||
            !gameKey ||
            suggestion.action.route !== '/games/' + gameKey
         ) {
            return null;
         }
         cta = { label, gameId };
...
export default function CompanionAvatar() {
   ...
   const {
      ...
      status: chatStatus,
      anchor: chatAnchor,
      setAnchor: setChatAnchor,
   } = useChatController();
   const { isInstructionAvatarVisible, isResultAvatarVisible } =
      useCompanionPresentation();
   ...
   const [moodPicked, setMoodPicked] = useState<CompanionMoodSelection | null>(
      null
   );
   ...
   const [coachingToastMessage, setCoachingToastMessage] = useState<
      string | null
   >(null);
   ...
   const contextResetTimerRef = useRef<ReturnType<typeof setTimeout> | null>(
      null
   );
   const coachingToastTimerRef = useRef<ReturnType<typeof setTimeout> | null>(
      null
   );
   ...
   useEffect(() => {
      ...
      getPersonalizationProfile(token)
         .then((profile) => {
            ...
            setProfileGender({
               userId,
               gender: isApiError(profile)
                  ? 'neutral'
                  : inferUserGender(profile),
            });
         })
         .catch(() => {
   ...
   useEffect(() => {
      const onCoachingToast = (event: Event) => {
         ...
         if (coachingToastTimerRef.current)
            clearTimeout(coachingToastTimerRef.current);
   ...
   const effectiveGender =
      token && profileGender?.userId === (user?.id ?? null)
   ...
   const messages = useMemo(() => {
      ...
      const gameKey =
         ctx === 'game'
            ? Object.values(GAME_ROUTE).find(
                 (k) => loc.pathname === `/games/${k}`
              )
            : undefined;
      ...
      const homeMood: Msg = {
         title: adaptHebrewForGender(
            t('companion.home.moodTitle'),
            effectiveGender,
            lang
         ),
         text: '',
         ...
         moodReplies: [
            {
               label: adaptHebrewForGender(
                  t('companion.home.moodAlert'),
                  effectiveGender,
                  lang
               ),
               value: 'alert',
      ...
      const planDomains = Array.from(
         new Set(
            (data?.plan?.items ?? [])
               .filter((item) =>
                  COGNITIVE_PROBLEMS.some(
                     (problem) => problem.id === item.domainId
                  )
               )
               .slice(0, 3)
      ...
      const homePlan: Msg | null =
         planDomains.length > 0
            ? {
                 text: adaptHebrewForGender(
                    formatSuggestionText(
                       t('companion.home.planRecommendation'),
                       {
      ...
      if (data && backendContext) {
         ...
         if (backendMessages) {
            return ctx === 'home'
               ? [...homeOpeningMessages, ...backendMessages]
               : backendMessages;
         }
      }
   ...
   const landInPlace = () => {
      ...
      if (el) {
         ...
         el.style.transform =
            frozen === 'none' ? 'translate(0px, 0px)' : frozen;
      }
      setFlight('land');
   };
   ...
   const measureAvatarAnchor = useCallback((): ChatAnchor | null => {
      ...
      if (roomAbove >= 260) {
         return {
            right,
            bottom: Math.max(12, Math.round(vh - rect.top + gap)),
         };
      }
   ...
   useEffect(() => {
      if (ctx !== 'home' || flight !== 'cruise' || !openAfterLaunch.current)
         return;
   ...
   useEffect(() => {
      ...
      if (open) {
         ...
         const t = setTimeout(
            () => {
               if (ctx === 'game')
                  setOpen(false); // no automatic game coaching rotation
               else if (idx < messages.length - 1)
                  setIdx(idx + 1); // next message
   ...
   if (isInstructionAvatarVisible || isResultAvatarVisible || isJourneyRoute)
      return null;
   ...
   return (
      ...
      >
         {coachingToastMessage && !suppressCompanion && ctx === 'game' && (
            <div
               className="companion-coaching-toast"
               role="status"
               aria-live="polite"
            >
               {adaptHebrewForGender(
                  coachingToastMessage,
                  effectiveGender,
                  lang
               )}
            </div>
         ...
         {open && msg && !suppressCompanion && (
            <div
               ...
               style={{
                  animationDuration: `${msg.durationMs ?? readMs(msg.text)}ms`,
               }}
            >
               <div
               ...
               >
                  ...
                  {msg.text && (
                     <p
                        className={
                           msg.title ? 'companion-reason' : 'companion-greet'
                        }
                     >
                        {msg.text}
                     </p>
         ...
         >
            {videoOk ? (
               ...
               <img
                  ...
                  alt="המאמן שלי"
                  onError={() =>
                     setBroken((b) => ({ ...b, [resolvedPose]: true }))
                  }
               />
            ) : (
               <svg
