...
function reasonHe(domain, gameHe) {
   ...
   if (domain.deteriorationFlag)
      return `זוהתה ירידה ב${dHe} — חיזוק ממוקד ב"${gameHe}".`;
   if (domain.trend === 'down')
      return `${dHe} במגמת ירידה — כדאי לתרגל ב"${gameHe}".`;
...
export function buildTrainingPlan(domains) {
   ...
   const weeklySessions = items.reduce(
      (sum, it) => sum + it.sessionsPerWeek,
      0
   );
...
export function targetMinutesForGame(domains, gameId) {
   ...
   const item = buildTrainingPlan(list).items.find(
      (it) => it.gameId === gameId
   );
   if (!item) {
      return { gameId, domainId, minutes: TIME.NEUTRAL, basis: 'neutral' };
